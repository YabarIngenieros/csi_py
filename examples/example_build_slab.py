...
def build_slab_structure():
    ...
    try:
        ...
        p2 = model.model.PointObj.AddCartesian(6, 0, 0)[0]
        p3 = model.model.PointObj.AddCartesian(6, 6, 0)[0]
        p4 = model.model.PointObj.AddCartesian(0, 6, 0)[0]
        ...
        # Puntos superiores
        p5 = model.model.PointObj.AddCartesian(0, 0, 3)[0]
        p6 = model.model.PointObj.AddCartesian(6, 0, 3)[0]
        p7 = model.model.PointObj.AddCartesian(6, 6, 3)[0]
        p8 = model.model.PointObj.AddCartesian(0, 6, 3)[0]
        ...
        for i, (pb, pt) in enumerate(zip(base_points, top_points), 1):
            col = model.model.FrameObj.AddByPoint(pb, pt)[0]
        ...
        area_name = model.model.AreaObj.AddByPoint(len(area_points), area_points)[0]
        
        # Asignar sección de losa
        model.model.AreaObj.SetProperty(area_name, 'LOSA20')
        ...
        # Combo de servicio
        model.add_load_combo('SERVICIO', 0)
        model.set_combo_case('SERVICIO', 'DEAD', 1.0)
        model.set_combo_case('SERVICIO', 'SUPER', 1.0)
        model.set_combo_case('SERVICIO', 'LIVE', 1.0)
        
        # Combo de resistencia (simplificado)
        model.add_load_combo('RESISTENCIA', 0)
        model.set_combo_case('RESISTENCIA', 'DEAD', 1.4)
        model.set_combo_case('RESISTENCIA', 'SUPER', 1.4)
        model.set_combo_case('RESISTENCIA', 'LIVE', 1.7)
